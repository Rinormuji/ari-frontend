...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        let userData = { username: formData.username, roles: response.data.roles ?? [] }
        try {
  ...
  const input = "w-full bg-white/5 border border-white/10 text-white placeholder-white/30 rounded-xl text-sm py-3 px-4 focus:ring-2 focus:ring-[#EFD391]/50 focus:border-[#EFD391] outline-none transition"
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-end">
              <Link to="/forgot-password" className="text-xs text-[#EFD391]/70 hover:text-[#EFD391] transition-colors">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 bg-[#EFD391] hover:bg-[#D9BF7B] disabled:opacity-50 text-black font-semibold rounded-xl transition-colors text-sm"
            >
          ...
          <p className="text-center text-sm text-white/40 mt-6">
            Nuk keni llogari?{' '}
            <Link to="/register" className="text-[#EFD391] hover:underline font-medium">
              Regjistrohu
            </Link>
          </p>
