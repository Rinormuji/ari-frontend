...
const ForgotPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage('Nëse kjo adresë është e regjistruar, udhëzimet janë dërguar me email.');
    } catch {
      setMessage('Kërkesa nuk mund të përfundohet për momentin. Ju lutemi, provoni përsëri.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-linear-to-br from-gray-100 to-gray-200 px-4 py-12">
      <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          <p className="text-sm text-gray-500 mb-6">Shkruani emailin tuaj për të marrë udhëzimet për ndryshimin e fjalëkalimit.</p>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="relative">
              <Mail size={17} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
              <input type="email" placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-[#EFD391]/40 focus:border-[#EFD391] outline-none transition" required />
            </div>
